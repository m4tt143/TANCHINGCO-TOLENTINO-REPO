...
public class GamePanel extends JPanel implements KeyListener, MouseListener, MouseMotionListener {
    ...
    private void update() {
        ...
        if (isMoving) {
            characterBobOffset = (float)Math.sin(animTick * 0.3) * 2f;
        } else {
            characterBobOffset *= 0.9f;
        }
    ...
    private void spawnHitSparks(Enemy e) {
        for (int i = 0; i < 15; i++) {
            ...
            int rv = 255;
            int gv = 160 + (int)(Math.random() * 95);
            int bv = 40 + (int)(Math.random() * 100);
    ...
    private void spawnBloodDecal(Enemy e) {
        ...
        if (bloodDecals.size() > 100) {
            bloodDecals.subList(0, bloodDecals.size() - 100).clear();
        }
    }
    ...
    private void applyUpgrade(int index) {
        ...
        switch (tag) {
            ...
            case "REGEN" -> player.regenTicksLeft = Integer.MAX_VALUE;
    ...
    protected void paintComponent(Graphics g) {
        ...
        for (int y = 0; y < GameFrame.HEIGHT; y += 2) {
            g2.drawLine(0, y, GameFrame.WIDTH, y);
        }
        // Global vignette
        for (int i = 0; i < 45; i++) {
    ...
    private void drawBackground(Graphics2D g) {
        // Darker base for torch contrast
        g.setColor(new Color(8, 18, 12)); g.fillRect(0,0,GameFrame.WIDTH,GameFrame.HEIGHT);
        ...
        for(int i=0;i<18;i++){
            ...
            g.setColor(new Color(18,38,22,60)); g.fillOval(bx,by,bw,bh);
        }
        for(int i=0;i<40;i++){
    ...
    private void drawGame(Graphics2D screenG) {
        // ── Render world to offscreen buffer ──
        ...
        // Camera-transformed world layer
        ...
        for (float[] p : particles) {
            ...
            g2.setColor(new Color(r, gv, bv, Math.max(0, Math.min(255, a))));
        ...
        g2.dispose(); // end camera transform

        // Lighting overlay (screen space, but lights positioned in world space)
        ...
        g.dispose(); // end buffer rendering

        // ── Post-process & blit to screen ──
    ...
    private void drawLighting(Graphics2D g, float camX, float camY) {
        BufferedImage darkness = new BufferedImage(GameFrame.WIDTH, GameFrame.HEIGHT, BufferedImage.TYPE_INT_ARGB);
        Graphics2D d = darkness.createGraphics();
    ...
    private void drawCrosshair(Graphics2D g, int cx, int cy) {
        ...
        int rv=(int)(80),gv=(int)(200+55*alpha),b=(int)(120);
        g.setColor(new Color(rv,gv,b,(int)(200*alpha)));
