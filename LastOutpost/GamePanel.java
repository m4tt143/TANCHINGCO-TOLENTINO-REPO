...
private void drawMenu(Graphics2D g) {
    ...
    // Floating dot particles (like the reference game)
    long t = System.currentTimeMillis();
    for (int i = 0; i < 40; i++) {
        double px = ((t * (0.003 + i * 0.001) + i * 211) % W);
        double py = ((t * (0.002 + i * 0.001) + i * 137) % H);
        int alpha = (int)(30 + 25 * Math.sin(t * 0.001 + i));
        g.setColor(new Color(150, 150, 200, alpha));
        g.fillRect((int) px, (int) py, 1, 1);
    }

    // Silhouette zombie shapes in background (decorative)
    g.setColor(new Color(30, 20, 40, 80));
    int[] zx = {120, 250, 550, 680, 750};
    int[] zy = {350, 420, 380, 440, 360};
    for (int i = 0; i < zx.length; i++) {
        // body
        g.fillOval(zx[i], zy[i] - 30, 22, 30);
        // head
        g.fillOval(zx[i] + 3, zy[i] - 50, 16, 18);
        // arms outstretched
        g.fillRect(zx[i] - 12, zy[i] - 22, 14, 5);
        g.fillRect(zx[i] + 22, zy[i] - 22, 14, 5);
    }

    // ── Big pixel-style title ──
    g.setFont(new Font("Monospaced", Font.BOLD, 58));
    FontMetrics fm = g.getFontMetrics();
    String t1 = "Last Outpost";
    // Shadow
    g.setColor(new Color(20, 80, 40, 100));
    g.drawString(t1, (W - fm.stringWidth(t1)) / 2 + 3, 118);
    // Main
    g.setColor(new Color(180, 220, 180));
    g.drawString(t1, (W - fm.stringWidth(t1)) / 2, 115);

    g.setFont(new Font("Monospaced", Font.BOLD, 24));
    fm = g.getFontMetrics();
    String t2 = "LAST  STAND";
    g.setColor(new Color(100, 160, 110));
    g.drawString(t2, (W - fm.stringWidth(t2)) / 2, 148);

    // Thin divider
    g.setColor(new Color(80, 120, 80, 120));
    g.fillRect(W / 2 - 180, 160, 360, 1);

    // ── Menu buttons (pixel bracket style like the reference game) ──
    String[] buttons   = { "NEW GAME", "CONTROLS", "QUIT" };
    int btnY = 200;
    int selectedBtn    = (int)((animTick / 40) % 3); // just for visual demo
    ...
    for (int i = 0; i < buttons.length; i++) {
        ...
        // Button background
        g.setColor(new Color(12, 20, 14));
        g.fillRect(bX, bY, bW, bH);

        // Pixel bracket corners (like [ ] in the reference)
        g.setColor(new Color(140, 200, 150));
        g.setStroke(new BasicStroke(2f));
        // Top-left corner
        g.drawLine(bX,      bY,      bX + 12, bY);
        g.drawLine(bX,      bY,      bX,      bY + 10);
        // Top-right corner
        g.drawLine(bX+bW,   bY,      bX+bW-12, bY);
        g.drawLine(bX+bW,   bY,      bX+bW,    bY+10);
        // Bottom-left corner
        g.drawLine(bX,      bY+bH,   bX+12,    bY+bH);
        g.drawLine(bX,      bY+bH,   bX,       bY+bH-10);
        // Bottom-right corner
        g.drawLine(bX+bW,   bY+bH,   bX+bW-12, bY+bH);
        g.drawLine(bX+bW,   bY+bH,   bX+bW,    bY+bH-10);
        g.setStroke(new BasicStroke(1f));

        // Button text
        g.setFont(new Font("Monospaced", Font.BOLD, 16));
        fm = g.getFontMetrics();
        g.setColor(new Color(180, 230, 185));
        g.drawString(buttons[i], (W - fm.stringWidth(buttons[i])) / 2, bY + 25);
    }
    ...
    if (highScore > 0) {
        ...
        g.drawString(hs, (W - fm.stringWidth(hs)) / 2, btnY + 3 * 58 + 30);
    }

    // Press ENTER hint
    double blink = Math.sin(animTick * 0.08) * 0.5 + 0.5;
    g.setFont(new Font("Monospaced", Font.PLAIN, 13));
    fm = g.getFontMetrics();
    String hint = "PRESS  ENTER  TO  START";
    g.setColor(new Color(100, 160, 110, (int)(120 + blink * 120)));
    ...
    g.setColor(new Color(50, 80, 55));
    g.drawString(cred, (W - fm.stringWidth(cred)) / 2, H - 12);
}
